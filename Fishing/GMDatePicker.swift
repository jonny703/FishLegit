...
class GMDatePicker: UIView {
    ...
    fileprivate func setup(_ parentVC: UIViewController) {
        ...
        let date1 = calendar.startOfDay(for: config.startDate!)
        
        let componet = calendar.dateComponents([.year], from: date1)
        let year = componet.year
        
//        let isoDate = "2016-04-14T10:44:00+0000"
        let isodate =  String(year!) + "-12-31T00:00:00+0000"
        ...
        let components1 = calendar.dateComponents([.second], from: date1, to: date2)
        
//        let secondsInMonth: TimeInterval = 30 * 24 * 60 * 60
        let  seconds: TimeInterval = Double(components1.second!)
        datePicker.maximumDate = Date(timeInterval: seconds, since: Date())
